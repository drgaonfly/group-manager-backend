...
export const transformDocumentImages = async (documents: any[], imageUrlFields: string | string[]): Promise<any> => {
  return await Promise.all(documents.map(async (doc) => {
    const docObject = doc.toObject();

    // 如果 imageUrlFields 是字符串，将其转换为数组
    const fieldsArray = Array.isArray(imageUrlFields) ? imageUrlFields : [imageUrlFields];
